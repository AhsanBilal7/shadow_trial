...
class DataLoaderTestAhsan(Dataset):
    def __init__(self, rgb_dir, img_options=None, target_transform=None):
        super(DataLoaderTestAhsan, self).__init__()
...
class DataLoaderVal(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Assume clean, noisy, and mask are your tensors
        if self.img_options is not None:
            size = self.img_options['patch_size']
            clean = F.interpolate(clean.unsqueeze(0), size=(size, size), mode='bilinear', align_corners=False).squeeze(0)
            noisy = F.interpolate(noisy.unsqueeze(0), size=(size, size), mode='bilinear', align_corners=False).squeeze(0)
            mask = F.interpolate(mask.unsqueeze(0), size=(size, size), mode='bilinear', align_corners=False).squeeze(0)
